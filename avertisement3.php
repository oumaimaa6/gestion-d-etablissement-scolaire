...
$stm=$conn->query("SELECT `date` from absence where justification='non' and code='$napogee' LIMIT 3");
$res=$stm->fetchAll(); 
$tab=array();
$stq=$conn->query("SELECT type from assiduite where type='avertissement' and code_etudiant='$napogee'");
while($val=$stq->fetch()){
$tab[]=$val['type'];
}
$n=count($tab)+1;
...
<head>
    ...
    <title>Avertisement</title>
</head>
<body>
<table style="position:absolute ;top:5px ; left:0px;">
<td><img  src="en-tete .png" style="width: 750px; "></td>
</table> 
<h2 style="position:absolute ;top:250px ; left:250px">Avertissement n°<?php echo $n ;?> </h2>
...
S'est absenté(e) sans aucune autorisation préalable et sans aucune justification : <br><br>
<?php foreach($res as $value){ echo "- Le ".date('d/m/Y', strtotime($value['date']))."<br>";} ?><br>
Par conséquent, et conformément à l'article 26 du règlement intérieur  de l'institut, la Direction de l'IFTSAU d'Oujda a décidé à son égard un avertissement</P>
